...
;(async () => {
  ...
  let content = "------------------------------"+"</br>"+([result["YouTube"],result["Netflix"],result["Disney"],result["Dazn"],result["Paramount"],result["Discovery"],result["ChatGPT"],result["Gemini"]]).join("</br></br>")
...
function testGemini() {
  ...
  $task.fetch(option).then(response => {
    ...
    console.log(`Gemini: statusCode = ${statusCode}`)
    if (statusCode === 200) {
      ...
      console.log(`Gemini: 未处理的状态码 ${statusCode}`)
    }
  }).catch(reason => {
    console.log(`Gemini 检测异常: ${reason}`)
