...
function random() {
    ...
    const cardId = `JCUT1${year}00${randomNum}`;

    // 直接写入 reactiveConfig，保证“随机生成”后的数据就是当前真值。
    // 这样后面只改一个字段时，其他字段不会被 updateUI 用旧默认值覆盖掉。
    setCardData({
        cardId,
        name: randomName,
        stuId: sid,
...
export async function generate() {
    ...
    const shortId = `${getChinesePinyinInitials(name)}${stuId.slice(-4)}`;
